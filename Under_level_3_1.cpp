...
void Under_Level_3_1::makeSprites()
{
	/*
...
void Under_Level_3_1::makePause(Music & music)
{
	/*
...
void Under_Level_3_1::moveAlice(double timer)
{
	/*
	...
	*/
	...
	if (AliceKick)
	{
		...
		timerAliseStay += timer;
		pointerAliceFire = 1;

		/// if time for kick is finish, we stop kicking and make timer equal 0
		if (timerAliseStay > FIRST_ALICE_FIRE_TIMER)
		{
			pointerAliceFire = 2;
		}
		if (timerAliseStay > SECOND_ALICE_FIRE_TIMER)
		{
			pointerAliceFire = 3;
		}
		if (timerAliseStay > THIRD_ALICE_FIRE_TIMER)
		{
			pointerAliceFire = 4;
		}
		if (timerAliseStay > FOURTH_ALICE_FIRE_TIMER)
		{
			pointerAliceFire = 5;
		}
		if (timerAliseStay > FIFTH_ALICE_FIRE_TIMER)
		{
			pointerAliceFire = 0;
			AliceKick = 0;
			timerAliseStay = 0;
	...
	if (AliceVector)
	{
		...
		if (AliceVector > 0)
		{
			AliceSprite.setTextureRect(IntRect(POSITION_ALICE_ARMOR_LEFT + ALICE_ARMOR_LENGTH * (1 + pointerRunningTexture), POSITION_ALICE_ARMOR_TOP + KICK_PHOTO_DISPLACEMENT * AliceKick, ALICE_ARMOR_LENGTH-1, ALICE_ARMOR_HIGH));
		}
		else
		{
			AliceSprite.setTextureRect(IntRect(POSITION_ALICE_ARMOR_LEFT + ALICE_ARMOR_LENGTH * (2 + pointerRunningTexture)-1, POSITION_ALICE_ARMOR_TOP + KICK_PHOTO_DISPLACEMENT * AliceKick, -ALICE_ARMOR_LENGTH, ALICE_ARMOR_HIGH));
		}
	}
	else
	{
		...
		{
			AliceSprite.setTextureRect(IntRect(POSITION_ALICE_ARMOR_LEFT, POSITION_ALICE_ARMOR_TOP + KICK_PHOTO_DISPLACEMENT * AliceKick, ALICE_ARMOR_LENGTH-1, ALICE_ARMOR_HIGH));
		}
		else
		{
			AliceSprite.setTextureRect(IntRect(POSITION_ALICE_ARMOR_LEFT + ALICE_ARMOR_LENGTH, POSITION_ALICE_ARMOR_TOP + KICK_PHOTO_DISPLACEMENT * AliceKick, -ALICE_ARMOR_LENGTH+1, ALICE_ARMOR_HIGH));
		}
	}
...
void Under_Level_3_1::moveClouds(double timer)
{
	/*
...
void Under_Level_3_1::moveDragon(double timer)
{
	/*
...
void Under_Level_3_1::checkFire(RenderWindow & window)
{
	/*
	...
	*/
	...
	{
		if (positionAliceX > positionDragonX - ONE_FIRE_DISPLACEMENT - ALICE_ARMOR_LENGTH &&
	...
	{
		if (positionAliceX > positionDragonX - TWO_FIRE_DISPLACEMENT - ALICE_ARMOR_LENGTH &&
	...
	{
		if (positionAliceX > positionDragonX - THREE_FIRE_DISPLACEMENT - ALICE_ARMOR_LENGTH &&
	...
	if (pointerDragonFire == 4)
	{
		if (((positionAliceX > positionDragonX - THREE_FIRE_DISPLACEMENT - ALICE_ARMOR_LENGTH &&
			...
			(positionAliceX > positionDragonX + DRAGON_PHOTO_LENGTH + ONE_FIRE_DISPLACEMENT - ALICE_ARMOR_LENGTH &&
				positionAliceX < positionDragonX + THREE_FIRE_DISPLACEMENT + DRAGON_PHOTO_LENGTH)) &&
				(positionAliceY > MIN_ALICE_DOWN_POSITION - ALICE_DISPLACEMENT_ABOUT_DRAGON_FIRE_BY_VERTICAL))
		{
			restart(window);
	...
	if (pointerDragonFire == 5)
	{
		...
		if (((positionAliceX > positionDragonX - THREE_FIRE_DISPLACEMENT - ALICE_ARMOR_LENGTH &&
			...
			(positionAliceX > positionDragonX + TWO_FIRE_DISPLACEMENT + DRAGON_PHOTO_LENGTH - ALICE_ARMOR_LENGTH &&
				positionAliceX < positionDragonX + THREE_FIRE_DISPLACEMENT + DRAGON_PHOTO_LENGTH)) &&
				(positionAliceY > MIN_ALICE_DOWN_POSITION - ALICE_DISPLACEMENT_ABOUT_DRAGON_FIRE_BY_VERTICAL))
		{
			restart(window);
...
void Under_Level_3_1::restart(RenderWindow & window)
{
	/*
...
void Under_Level_3_1::checkIntersect(RenderWindow & window)
{
	/*
	...
	*/
	...
	int rightX = round(min(positionAliceX + ALICE_ARMOR_LENGTH, positionDragonX + DRAGON_PHOTO_LENGTH));
	int upY = round(max(positionAliceY, positionDragonY));
	int downY = round(min(positionAliceY + ALICE_ARMOR_HIGH, positionDragonY + DRAGON_PHOTO_HIGH));
	...
	if (downY - upY > rightX - leftX)
	{
		...
		{
			positionAliceX = positionDragonX - ALICE_ARMOR_LENGTH;
		}
...
void Under_Level_3_1::moveObjects(RenderWindow & window, double timer)
{
	/*
...
void Under_Level_3_1::drawPicture(RenderWindow & window)
{
	/*
	...
	*/
	...
	bool AliceFireVector = (positionAliceX < positionDragonX);

	if (AliceVector)
	{
		AliceFireVector = AliceVector > 0;
	}

	if (pointerAliceFire == 1)
	{
		if (AliceFireVector)
		{
			fireSprite.setTextureRect(IntRect(POSITION_DRAGON_FIRE_PHOTO_LEFT + DRAGON_FIRE_PHOTO_LENGTH - 1, POSITION_DRAGON_FIRE_PHOTO_TOP, -DRAGON_FIRE_PHOTO_LENGTH +1, DRAGON_FIRE_PHOTO_HIGH));
			fireSprite.setPosition(positionAliceX + ALICE_ARMOR_LENGTH, positionAliceY + ALICE_FIRE_DISPLACEMENT);

			window.draw(fireSprite);
		}
		else
		{
			fireSprite.setTextureRect(IntRect(POSITION_DRAGON_FIRE_PHOTO_LEFT - 1, POSITION_DRAGON_FIRE_PHOTO_TOP, DRAGON_FIRE_PHOTO_LENGTH -1, DRAGON_FIRE_PHOTO_HIGH));
			fireSprite.setPosition(positionAliceX - DRAGON_FIRE_PHOTO_LENGTH, positionAliceY + ALICE_FIRE_DISPLACEMENT);

			window.draw(fireSprite);
		}
	}

	if (pointerAliceFire == 2)
	{
		if (AliceFireVector)
		{
			fireSprite.setTextureRect(IntRect(POSITION_DRAGON_FIRE_PHOTO_LEFT + 3 * DRAGON_FIRE_PHOTO_LENGTH - 3, POSITION_DRAGON_FIRE_PHOTO_TOP, -2 * DRAGON_FIRE_PHOTO_LENGTH + 2, DRAGON_FIRE_PHOTO_HIGH));
			fireSprite.setPosition(positionAliceX + ALICE_ARMOR_LENGTH, positionAliceY + ALICE_FIRE_DISPLACEMENT);
			window.draw(fireSprite);
		}
		else
		{
			fireSprite.setTextureRect(IntRect(POSITION_DRAGON_FIRE_PHOTO_LEFT + DRAGON_FIRE_PHOTO_LENGTH - 3, POSITION_DRAGON_FIRE_PHOTO_TOP, 2 * DRAGON_FIRE_PHOTO_LENGTH - 2, DRAGON_FIRE_PHOTO_HIGH));
			fireSprite.setPosition(positionAliceX - DRAGON_FIRE_PHOTO_LENGTH * 2, positionAliceY + ALICE_FIRE_DISPLACEMENT);
			window.draw(fireSprite);
		}
	}

	if (pointerAliceFire == 3)
	{
		if (AliceFireVector)
		{
			fireSprite.setTextureRect(IntRect(POSITION_DRAGON_FIRE_PHOTO_LEFT + 9 * DRAGON_FIRE_PHOTO_LENGTH - 7, POSITION_DRAGON_FIRE_PHOTO_TOP, -3 * DRAGON_FIRE_PHOTO_LENGTH + 3, DRAGON_FIRE_PHOTO_HIGH));
			fireSprite.setPosition(positionAliceX + ALICE_ARMOR_LENGTH, positionAliceY + ALICE_FIRE_DISPLACEMENT);
			window.draw(fireSprite);
		}
		else
		{
			fireSprite.setTextureRect(IntRect(POSITION_DRAGON_FIRE_PHOTO_LEFT + 3 * DRAGON_FIRE_PHOTO_LENGTH - 7, POSITION_DRAGON_FIRE_PHOTO_TOP, 3 * DRAGON_FIRE_PHOTO_LENGTH - 3, DRAGON_FIRE_PHOTO_HIGH));
			fireSprite.setPosition(positionAliceX - 3 * DRAGON_FIRE_PHOTO_LENGTH, positionAliceY + ALICE_FIRE_DISPLACEMENT);
			window.draw(fireSprite);
		}
	}

	if (pointerAliceFire == 4)
	{
		if (AliceFireVector)
		{
			fireSprite.setTextureRect(IntRect(POSITION_DRAGON_FIRE_PHOTO_LEFT + 11 * DRAGON_FIRE_PHOTO_LENGTH - 8, POSITION_DRAGON_FIRE_PHOTO_TOP, -2 * DRAGON_FIRE_PHOTO_LENGTH + 2, DRAGON_FIRE_PHOTO_HIGH));
			fireSprite.setPosition(positionAliceX + ALICE_ARMOR_LENGTH + ONE_FIRE_DISPLACEMENT, positionAliceY + ALICE_FIRE_DISPLACEMENT);
			window.draw(fireSprite);
		}
		else
		{
			fireSprite.setTextureRect(IntRect(POSITION_DRAGON_FIRE_PHOTO_LEFT + 9 * DRAGON_FIRE_PHOTO_LENGTH - 8, POSITION_DRAGON_FIRE_PHOTO_TOP, 2 * DRAGON_FIRE_PHOTO_LENGTH + 2, DRAGON_FIRE_PHOTO_HIGH));
			fireSprite.setPosition(positionAliceX - 2 * DRAGON_FIRE_PHOTO_LENGTH - ONE_FIRE_DISPLACEMENT, positionAliceY + ALICE_FIRE_DISPLACEMENT);
			window.draw(fireSprite);
		}
	}

	if (pointerAliceFire == 5)
	{
		if (AliceFireVector)
		{
			fireSprite.setTextureRect(IntRect(POSITION_DRAGON_FIRE_PHOTO_LEFT + 12 * DRAGON_FIRE_PHOTO_LENGTH - 8, POSITION_DRAGON_FIRE_PHOTO_TOP, -DRAGON_FIRE_PHOTO_LENGTH + 1, DRAGON_FIRE_PHOTO_HIGH));
			fireSprite.setPosition(positionAliceX + ALICE_ARMOR_LENGTH + TWO_FIRE_DISPLACEMENT, positionAliceY + ALICE_FIRE_DISPLACEMENT);
			window.draw(fireSprite);
		}
		else
		{
			fireSprite.setTextureRect(IntRect(POSITION_DRAGON_FIRE_PHOTO_LEFT + 11 * DRAGON_FIRE_PHOTO_LENGTH - 8, POSITION_DRAGON_FIRE_PHOTO_TOP, DRAGON_FIRE_PHOTO_LENGTH - 1, DRAGON_FIRE_PHOTO_HIGH));
			fireSprite.setPosition(positionAliceX - DRAGON_FIRE_PHOTO_LENGTH - TWO_FIRE_DISPLACEMENT, positionAliceY + ALICE_FIRE_DISPLACEMENT);
			window.draw(fireSprite);
		}
	}
...
bool Under_Level_3_1::switchEvent(Event event, RenderWindow & window, Music & music)
{
	/*
...
void Under_Level_3_1::checkAliceFire()
{
	if (!pointerAliceFire)
	{
		return;
	...
	double positionfireX = fireSprite.getPosition().x;
	double positionfireY = fireSprite.getPosition().y;
	double fireLength;
	double fireHigh = DRAGON_FIRE_PHOTO_HIGH;

	if (pointerAliceFire == 1 || pointerAliceFire == 5)
	{
		fireLength = DRAGON_FIRE_PHOTO_LENGTH;
	}
	else
	if (pointerAliceFire == 2 || pointerAliceFire == 4)
	{
		fireLength = DRAGON_FIRE_PHOTO_LENGTH*2;
	}
	else
	{
		fireLength = DRAGON_FIRE_PHOTO_LENGTH * 3;
	}

	double _Left = max(positionDragonX, positionfireX);
	double _Right = min(positionDragonX + DRAGON_PHOTO_LENGTH, positionfireX + fireLength);
	double _Up = max(positionDragonY, positionfireY);
	double _Down = min(positionDragonY + DRAGON_PHOTO_HIGH, positionfireY + fireHigh);
...
void Under_Level_3_1::checkTime(RenderWindow & window)
{
	/*
...
void Under_Level_3_1::game(RenderWindow & window, Music & music)
{
	/*
...
Under_Level_3_1::Under_Level_3_1()
{
}
