...
void Cut_Scene_5_1::makeSprites()
{
	/*
	...
	*/
	...
	backGroundSprite.setTextureRect(IntRect(POSITION_KINDERGARDEN_BACKGROUND_LEFT, POSITION_KINDERGARDEN_BACKGROUND_TOP, BACKGROUND_PHOTO_LENGTH, BACKGROUND_PHOTO_HIGH));

	groundSprite.setTexture(texturesEnviroment);
	groundSprite.setTextureRect(IntRect(POSITION_KINDERGARDEN_GROUND_LEFT, POSITION_KINDERGARDEN_GROUND_TOP, GROUND_PHOTO_LENGTH, GROUND_PHOTO_HIGH));
	...
	texturesDargon.loadFromFile("textures\\OtherCharactersTexture.png");

	ArtemSprite.setTexture(texturesDargon);
	ArtemSprite.setTextureRect(IntRect(POSITION_ARTEM_LEFT, POSITION_ARTEM_TOP, ARTEM_SPRITE_LENGTH, ARTEM_SPRITE_HIGH));
	ArtemSprite.setPosition(600, MIN_ALICE_DOWN_POSITION);

	ArtemHeadSprite.setTexture(texturesDargon);
	ArtemHeadSprite.setTextureRect(IntRect(POSITION_ARTEM_HEAD_PHOTO_LEFT, POSITION_ARTEM_HEAD_PHOTO_TOP, HEAD_PHOTO_LENGTH, HEAD_PHOTO_HIGH));

	return;
}

void Cut_Scene_5_1::drawDialog(RenderWindow & window)
{
	/*
	...
	*/
	...
	{
		/// if there is Artems replic
		if (i > 1)
		{
			/// set replic field color, size, position, set text string, position and draw
			replicField.setFillColor(Color::Green);
			ArtemHeadSprite.setPosition(POSITION_SQUIRREL_HEAD_SPRITE, 50 * (i + 1) + 5);
			window.draw(ArtemHeadSprite);
...
void Cut_Scene_5_1::moveObjects(double timer)
{
	/*
...
bool Cut_Scene_5_1::switchEvent(Event event, RenderWindow & window, Music & music)
{
	/*
	...
	*/
	...
	if (event.type == Event::MouseButtonPressed && event.mouseButton.button == Mouse::Left)
	{
		...
		if (timerStay > WAITING_AFTER_END_OF_MOVING)
		{
			...
			if (dialogPointer == COUNT_MOUSE_CLICK_CUT_SCENE_5_1)
			{
				return 1;
			}
		}
...
void Cut_Scene_5_1::moveClouds(double timer)
{
	/*
...
void Cut_Scene_5_1::moveAlice(double timer)
{
	/*
...
void Cut_Scene_5_1::makeTextTTC()
{
	/*
...
void Cut_Scene_5_1::checkTime(RenderWindow & window)
{
	/*
...
void Cut_Scene_5_1::drawPicture(RenderWindow & window)
{
	/*
...
void Cut_Scene_5_1::play(RenderWindow & window, Music & music)
{
	/*
